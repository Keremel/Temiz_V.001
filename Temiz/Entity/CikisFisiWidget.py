...
from .CikisFisi_ui import Ui_Form


class CikisFisiWidget(QWidget, Ui_Form):
    ...
    def init_ui(self):
        ...
        self.pushButton_hepsinikaydet.clicked.connect(self.save_cikis_fisi)
        ...
        self.tableWidget.setHorizontalHeaderLabels(
            ['Stok Kodu', 'Stok Adı', 'Birim', 'Birim Fiyat', 'Çıkacak Miktar', 'Toplam Tutar', 'Kalan Miktar'])
    ...
    def set_auto_fis_no(self):
        ...
        if last_fis_no:
            ...
            new_fis_no = 'CF00001'
        self.lineEdit_fisno.setText(new_fis_no)
    ...
    def search_stock_code(self):
        try:
            ...
            self.current_stock_info = self.cikis_fisi_service.get_stock_info(stokkod)
            if self.current_stock_info:
                stokad = self.current_stock_info['StokAd']
                birim = self.current_stock_info['Birim']
                birimfiyat = float(self.current_stock_info['SatisFiyati'])
                ...
                self.current_stock_info['SatisFiyati'] = birimfiyat
    ...
    def add_product(self):
        try:
            ...
            if miktar > aktifmiktar:
                QMessageBox.critical(self, 'Hata', f'Çıkacak miktar aktif miktarı aşamaz. Maksimum: {aktifmiktar}')
                return
    ...
    def save_cikis_fisi(self):
        try:
            ...
            for row in range(row_count):
                ...
                cikis_fisi = CikisFisi(fis_no, fis_tarihi, stokkod, stokad, birim, miktar, birim_fiyat, toplam_tutar,
                                       kalan_miktar)
                self.cikis_fisi_service.save_cikis_fisi(fis_no, fis_tarihi, stokkod, stokad, birim, miktar, birim_fiyat,
...
if __name__ == '__main__':
    ...
    service = CikisFisiService(None)  # Replace None with actual service initialization
    widget = CikisFisiWidget(service)
    widget.show()
    sys.exit(app.exec_())
